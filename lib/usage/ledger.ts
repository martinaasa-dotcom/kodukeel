 ...
 *   SCENE   one turn of one conversation. The booking is per turn (§16) and
 *           since ADR-025 amendment 2 a composed run asks for nearly every
 *           beat rather than only the ones the bank has no line for, so a
 *           whole scene is about ten of these where it used to be two or
 *           three. At the base it would be one conversation a day, which is
 *           not an allowance, it is a wall. Four times the base is three or
 *           four conversations, and the burst is doubled because a turn is a
 *           sentence and somebody talking is not somebody idling.
 ...
 *
 * The two above ration one learner. Neither says anything about which *kind*
 * of call gets the money when there is not enough for all of them, and until
 * scenes composed every beat that question did not arise: the tutor was the
 ...
 * somebody can have several in an evening, so on a small budget an afternoon
 * of role-play can reach the global cap and the next person to ask Anu a
 * question is told the deployment has run out.
 *
 * `globalShare` is the fraction of `dailyMicrosGlobal` a kind may reach before
 * it yields. It is not a reservation for anybody: what is left over is not
 * held back from scenes and then wasted, it is simply the point past which a
 * scene stops being the thing that spends it.
 *
 ...
 * under it and the other does not, the one with the floor is the one that
 * gives way.
 *
 * Half rather than a smaller share, because scene composition is the thing
 * this budget is mostly *for*: a share that made scenes yield early would
 * protect a tutor nobody had asked anything.
 */
  ...
  try {
    ...
    const scaled = {
      ...
      /*
        ...
        snapshot with no `kind` in it so that the sentence it refuses with is
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      /*
        ...
        and says "this deployment has reached its shared daily budget", which is
        the honest sentence when the whole budget really is gone. A kind that
        yields at half of it reaches that branch with half the day's money still
        unspent, and telling somebody the deployment is out when Anu is about to
        answer perfectly well is the kind of small wrongness a reader catches
    ...
    return {
      ...
      reason: "GLOBAL_SPEND",
      /*
        Names no one feature, for the reason `checkQuota`'s own messages do
        not: this is reached by a conversation, a photograph and a question
        alike, and it used to say "Anu is unavailable" to a learner standing
        at a reception desk.
      */
      message: "That could not be asked just now. The usage ledger could not be read.",
 ...
 *
 * Only reached by a kind whose `globalShare` is below one, which is scene
 * composition alone: it yields so that a question to Anu, which has no rung
 * under it, is still answerable at the end of a day of role-play.
 */
