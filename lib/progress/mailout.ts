 ...
 *
 * AND THE WALK HAS TO TURN ON WHATEVER SCHEDULE THE DEPLOYMENT HAS. This used
 * to turn the page with the hour since the epoch, reasoned about an hourly
 * schedule, while `vercel.json` fired once a day at 16:00 UTC, because a
 * Hobby plan refuses an hourly expression. Between two daily runs that key
 ...
 * days. Hourly runs, which `vercel.json` now gives as twenty-four daily
 * entries, reach twenty-four pages in a day, which is everybody up to
 * forty-eight thousand learners, and beyond that every page within as many
 * days as there are pages. No function of the clock can move a page every hour
 * and also a page every day, since the first moves twenty-four a day, and this
 * key is right under both. Deterministic, so it needs no stored cursor and
