 ...
 *
 * `practises(scene).join(", ")` was the only caller, and a plain comma join
 * turned two items into something that reads as one: "a word off your card,
 * the polite you" looks like the second half is describing the first, as
 * though the card itself were the polite you. "and" before the last item is
 * what a sentence needs to say two separate things are both required.
 */
