      ...
      return {
        ...
        */
        body: [
          ...
          /*
            MAIN'S CAPITALISED HOOK, EXCEPT WHERE THE HOOK IS THE PLAIN WORD.

            Two sessions found "It is the ending for into. into." on the same
            day. Capitalising the hook is the fix that keeps the reference
            page's own examples, which are the better copy in twelve of the
            fourteen: "It is the ending for in. In the house, in March, in a
            good mood." lands where a definition would not.

            The illative is where it does not, because its hook *is* the plain
            word: capitalised, the note reads "It is the ending for into.
            Into.", which is the same word twice and was the fault the whole
            way. There `summary` is the field that carries a sentence, so it
            answers instead.
          */
          opensSentence(saysMore(note) ?? note?.summary ?? ""),
        ].filter(Boolean).join(" "),
...
/**
 * The hook, where it says something the plain word did not.
 *
 * `plain` is the case in one English word and the hook is meant to be the
 * example under it, but for the illative the two are the same word: "into"
 * and "into.", so the note said it twice. Compared without punctuation or
 * case, and nothing else about either is touched.
 */
function saysMore(note: { plain: string; englishHook?: string } | undefined): string | undefined {
  const hook = note?.englishHook;
  if (!hook || !note) return hook;
  const bare = (text: string) => text.replace(/[.,;:!?]/g, "").trim().toLowerCase();
  return bare(hook) === bare(note.plain) ? undefined : hook;
}
