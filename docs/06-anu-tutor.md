...
| Choice | Why |
|---|---|
| A chain, not a model | Anu reads her own chain, `TUTOR_MODEL` on Gemini with `TUTOR_FALLBACK_MODEL` on Groq behind it, both pinned (`PURPOSE_CHAINS` in `lib/tutor/provider.ts`). A deployment holding either free key has a tutor, and one holding neither says so rather than falling to a model nobody measured on her questions |
