...
| Choice | Why |
|---|---|
| A chain, not a model | Anu's chain is pinned in `PURPOSE_CHAINS.tutor`: `TUTOR_MODEL` on Gemini, then `TUTOR_FALLBACK_MODEL` on Groq, each only where its key is set. Both have a free tier, so a deployment with no paid key still has a tutor |
