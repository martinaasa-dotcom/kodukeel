...
| Choice | Why |
|---|---|
| A chain, not a model | `resolveProviders()` returns every key in `.env`, free first: Groq, then Gemini, then Anthropic and OpenAI as a bounded paid tail. A deployment with no paid key still has a tutor |
