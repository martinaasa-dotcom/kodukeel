...
| 5.8 Information security in project management | Security rules are asserted in CI rather than reviewed by hand. A rule with no assertion is treated as a comment. The invariant suite runs on every push. | `scripts/test-invariants.ts` | Implemented |
...
| 5.16 Identity management | Supabase Auth: Google sign-in, mailed links, and SAML SSO for the domains in `SSO_DOMAINS`. Sign-in allowlist optional by address or domain. | `lib/auth/access.ts`, `lib/auth/sso.ts` | Implemented |
...
| 8.20 to 8.22 Network security and segregation | The application has one network boundary and no internal network of its own. Keyed third parties reachable only from the server, enforced by an invariant and by a CSP whose `connect-src` names `'self'` and the deployment's own Supabase project and nothing else. Where Google sign-in is configured the browser loads Google's script and frame from `accounts.google.com`. | `lib/security/headers.ts` | Implemented |
...
| 8.33 Test information | The fixtures in the repository are generated by scripts rather than exported from a deployment. The scripts that destroy data on purpose, the demo and load fixtures and several browser suites among them, call `requireLocalDatabase`, which refuses a non-local database unless `KODUKEEL_ALLOW_REMOTE_DB` is set; `scripts/e2e.mjs` and the other suites do not call it. There is no written policy forbidding production data in testing. | `scripts/lib/local-db.mjs`, `scripts/demo-data.ts` | Partial |
