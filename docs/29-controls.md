...
| 5.8 Information security in project management | Security rules are asserted in CI rather than reviewed by hand. A rule with no assertion is treated as a comment. Every invariant runs on every push and the run prints its own total: 426 when this row was last checked, on 2026-09-25. | `scripts/test-invariants.ts` | Implemented |
...
| 8.25 Secure development lifecycle | TypeScript strict with `noUncheckedIndexedAccess`, lint in the build rather than only in CI, a hermetic unit suite gating every commit, the invariant suite (426 on 2026-09-25), and browser suites covering every route. | `next.config.ts`, `.github/workflows/ci.yml` | Implemented |
