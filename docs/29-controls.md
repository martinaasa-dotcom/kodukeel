...
| 5.8 Information security in project management | Security rules are asserted in CI rather than reviewed by hand. A rule with no assertion is treated as a comment. The whole invariant suite runs on every push and prints its own total, 421 on the day this line was written. | `scripts/test-invariants.ts` | Implemented |
