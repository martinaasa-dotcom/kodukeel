...
| 8.20 to 8.22 Network security and segregation | The application has one network boundary and no internal network of its own. Third parties reachable only from the server, enforced by the CSP, whose `connect-src` is this origin plus, where one is configured, the Supabase project the browser signs in through, and by an invariant. | `lib/security/headers.ts` | Implemented |
