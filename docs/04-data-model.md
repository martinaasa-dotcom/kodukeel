...
| `CourseStep` | One step of a planned course day the learner finished (`lib/course/`). Only the steps no log can rebuild: meeting the day's words and the closing review are read off the review log instead and are never written here. Which day somebody is on is derived from these rows rather than stored, so there is no day pointer to drift. Append-only, and the unique key makes a second press a no-op. |
| `EmailSend` | One letter this deployment sent, and when, on the learner's own clock. Append-only, because it is the only record that somebody was written to and every decision about the next letter is read from it: the per-kind gap, the weekly ceiling and "have they had one today" are all questions about history, and a counter would answer none of them. Holds no subject, no body and nothing about whether it was opened, because there is no tracking pixel in this app. |
