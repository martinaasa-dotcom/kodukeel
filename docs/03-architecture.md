...
| AI | no SDK: one HTTP client over a chain of providers | `lib/tutor/provider.ts`: Groq, then Gemini, with Anthropic and OpenAI behind the fallback budget, and `PURPOSE_CHAINS` pinning each purpose; `13-mvp-status.md` §2 is where the chain was first decided |
