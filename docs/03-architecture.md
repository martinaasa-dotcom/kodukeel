...
| AI | no SDK: one HTTP client over a chain of providers | `lib/tutor/provider.ts`: Groq, then Gemini, then Anthropic and OpenAI while the fallback budget has room, and a chain of its own per purpose (CLAUDE.md, Model configuration) |
