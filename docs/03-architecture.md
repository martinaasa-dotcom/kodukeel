...
| AI | no SDK: one HTTP client over a chain of providers | `lib/tutor/provider.ts`: a pinned chain per purpose (Anu, scenes, graders) and a general chain of Groq, then Gemini, then a budget-gated Anthropic and OpenAI, per `13-mvp-status.md` §2 |
