...
export async function setEmailKind(input: { kind: string; on: boolean }) {
  ...
  */
  ...
  await changeEmailPrefs(ownerId, (current) =>
    input.on === true ? switchOn(current, kind) : switchOff(current, [kind]),
  );
  revalidatePath("/settings");
...
export async function completeOnboarding(input: {
  ...
  */
  ...
  if (busy) return busy;
  /*
    The goals object used to skip the schema `saveLearningGoals` holds the same
    fields to, and the unit list was read as whatever array arrived. Both are
    read here, before anything is written, so a shape that fails is refused
    whole rather than half-saved.
  */
