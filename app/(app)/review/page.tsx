...
export default async function ReviewPage({
  ...
  */
  const [due, freshPool, totalCards, level, mode] = await Promise.all([
    ...
    prisma.card.findMany({
      // Inside the module, only a word the module has taught: see above, and
      // `unseenWhere`, which the module's own closing count reads too.
      where: unseenWhere(ownerId, now, scope?.lemmas ?? null),
