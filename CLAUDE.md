...
on the sentence, so the line was already in the dictionary one entry over. The review queue and the
sprint read the borrowed pool behind the word's own, which costs no call and no write.
