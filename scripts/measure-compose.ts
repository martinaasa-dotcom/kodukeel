...
for (const scene of SCENES) {
  ...
  for (const beat of scene.beats) {
    liveChars += composeLive({ move: beat.move, they: beat.they, avoid: [] }).length;
    beats += 1;
  }
}
...
const turnTokens = Math.round(
  composeMessages(EXCHANGES).map((m) => m.content).join("\n").length / CHARS_PER_TOKEN.estonianList,
);
/*
...
line("answer, capped at", `${COMPOSE_MAX_TOKENS} tokens, about ${answerTokens} used`);
